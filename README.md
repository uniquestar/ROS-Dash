...
| DHCP | Active DHCP leases with hostname, IP, MAC, Client ID, status, lease type, and switch port location. Click any column header to filter by that column; multiple column filters can be active simultaneously. Users with dhcp:write permission can Reserve (make static) or Release (remove static) leases directly from the table |
